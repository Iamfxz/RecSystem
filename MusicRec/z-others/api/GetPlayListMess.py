...
class PlayList:
    def __init__(self):
        # 所有歌单的 id 和 歌单名 的保存文件
        self.playlist_file = "./newdata/playlist_url/playlist_id_name_all.txt"
        # 获取出错的 歌单id 保存文件
        self.error_id_file = "./newdata/error_playlist_ids.txt"
        # 歌单 创造者 信息
        self.creator_mess = "./newData/user_mess/"
        # 每个歌单的原始信息
        self.playlist_mess = "./newData/playlist_mess/"
        # 歌单包含的歌曲的原始信息
        self.trackid_mess = "./newData/song_mess/"
    ...
    # 每个歌单的内容进行格式化处理 写入文件
    # 需要获取的信息: 歌单信息、创建者信息、歌单中歌曲信息
    def getFormatPlayListMess(self, json_line):
        # 创建者信息
        # 用户id，昵称，生日，性别，省份，城市，类型，标签，头像链接，用户状态，账号状态，djStatus,vipStatus，签名
        ...
        self.writeToFile(self.trackid_mess + "pl_sing_id.txt", str(playlist["id"]) + "\t" + ",".join(t_list))
