...
class RecSong:
    def __init__(self):
        self.playlist_mess_file = "../tomysql/newData/pl_mess_all.txt"
        self.playlist_song_mess_file = "../tomysql/newData/pl_sing_id.txt"
        self.song_mess_file = "../tomysql/newData/song_mess_all.txt"
    ...
    def UserSimilarityBest(self):
        # 得到每个item被哪些user评价过，构建倒排表
        item_users = dict()
        for user_id, items in self.user_song_dict.items():
            for item in items.keys():
                item_users.setdefault(item, set())
                if self.user_song_dict[user_id][item] > 0:
    ...
    # 为每个用户推荐歌曲 user_song_prefer.json
    def recommend_song(self):
        # 记录用户对歌手的评分
        user_song_score_dict = dict()
        if os.path.exists("newData/user_song_prefer.json"):
            user_song_score_dict = json.load(open("newData/user_song_prefer.json", "r", encoding="utf-8"))
            ...
            for user_sim in self.user_sim[user].keys():
                ...
                for song in self.user_song_dict[user_sim].keys():
                    ...
                    user_song_score_dict[user][song] += self.user_sim[user][user_sim] * self.user_song_dict[user_sim][
                        song]
        json.dump(user_song_score_dict, open("newData/user_song_prefer.json", "w", encoding="utf-8"))
        print("用户对歌曲的偏好计算完成！")
        return user_song_score_dict
    
    # 写入文件
    def write_to_file(self):
        fw = open("newData/user_song_prefer.txt", "w", encoding="utf-8")
