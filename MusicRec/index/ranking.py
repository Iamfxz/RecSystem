...
def rankResult(request):
    ...
    result["code"] = 1
    result["data"] = dict()
    result["data"]["playlist"] = rankPlayList(u_id)
...
# 歌单排行榜
def rankPlayList(u_id):
    result_pl = list()
    play_lists = UserPlayListRec.objects.filter(user=u_id).order_by("-sim")
...
def rankSong(u_id):
    result_song = list()
    songs = UserSongRec.objects.filter(user=u_id).order_by("-sim")
    for song in songs:
        ...
        if s_id == "0":
            continue
        singers = Sing.objects.filter(sing_id=s_id)
        if singers.exists():
            singer = singers[0]
            result_song.append({
                "song_id": one.song_id,
...
def rankSinger(u_id):
    result_sing = list()
    sings = UserSingRec.objects.filter(user=u_id).order_by("-sim")
    for sing in sings:
