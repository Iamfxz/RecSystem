...
class GetSongMess:
    def __init__(self):
        ...
        # 保存 获取失败的id
        self.error_ids = list()
        self.error_ids_file = "./newdata/song_mess/error_ids_1.txt"
        
        # 记录所有id的文件 合并 获取所有 歌曲id
        self.ids_all_file = "./newdata/song_mess/"
    ...
    def getSongIDs(self):
        ...
        ids_list = set()
        print(" 获取所有的歌曲id信息 ...")
        for line in open(self.ids_all_file + "pl_sing_id.txt", "r").readlines():
            result = line.split("	")
            try:
                for id in result[1].strip().split(","):
                    ids_list.add(id)
    ...
    def getSongMess(self):
        ...
        for id in self.song_ids:
            try:
                ...
                try:
                    ...
                    lysic = "null"
                writeToFile(self.ids_all_file + "song_mess_all.txt",
                            " |+| ".join(url_1_list + url_2_list + url_3_list))
                writeToFile(self.ids_all_file + "song_lyric_mess_all.txt", str(res_1_json["id"]) + "\t" + lysic)
                i += 1
            except Exception as e:
                print("error: %s" % e)
        ...
        if self.error_ids.__len__() != 0:
            ...
            writeToFile(self.error_ids_file, ",".join(self.error_ids))
        print("歌曲信息获取完成 ...")
    ...
    def filterId(self):
        ...
        for one in self.song_ids:
            if one not in ids_list_not:
                print(i)
                i += 1
                writeToFile(self.ids_all_file + "not_get_ids_%s.txt" % str(int(i / 50000)), one)
